...
#### Step -1: 書き込み権限を先に確保する（何よりも先に行う）

**この手順を飛ばすと、記事をすべて書き上げてコミットしたあとに push だけが
拒否され、そのセッションの成果が丸ごと失われる。** 2026年9月3日の朝の回で
実際に起きた。4本の記事を書き、検査をすべて通過し、コミットまで済ませたのに
`022740mix-spec/AI-News is not in this session's authorized repository set`
で push が拒否され、記事は復旧できなかった。

**リポジトリが public なので clone は通る。push だけが落ちる。**
だから最後まで気づけない。調査・執筆・検証をすべて終えてから捨てることになる。

したがって、調査を始める前に次を行う。

1. `add_repo` ツール（Claude Code Remote の MCP）を
   `owner: "022740mix-spec"`, `repo: "AI-News"`, `access: "push"` で呼ぶ
2. 成功したら、返ってきた作業ディレクトリの clone で以後の作業を行う
3. **失敗した場合、または `add_repo` ツールが使えない場合は、記事を書き始めない。**
   「リポジトリへの書き込み権限を確保できなかった」とだけ報告して終える。
   書いてから捨てるのが最も損失の大きい終わり方である
...
| | 誰が | 何をする |
|---|---|---|
| 恒久 | 人間 | Routine の編集画面でリポジトリを選ぶ。MCP からは設定できない |
| 保険 | Routine 自身 | Step -1 で `add_repo`（`access: "push"`）を呼び、失敗したら書き始めない |
