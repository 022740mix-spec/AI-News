...
export const MODEL_COMPARISON = [
  ...
  { name: "Claude Mythos 5.1", rating: 5, summary: "Anthropic 最上位の更新版（9/1）。Fable 5.1 と同じ破壊的変更3件を含む。Terminal-Bench 4.0 で60.9。TB 4.0 は表の他モデル（2.0/2.1）と版が違うため列には入れていない", swe: null, swePro: null, terminalBench: null, osworld: null, browseComp: null, gpqa: null, aime: null, hle: null },
  { name: "Claude Fable 5", rating: 5.0, summary: "Anthropic 一般公開フラッグシップ（6/9 GA）。Mythos クラスを一般利用向けに安全化。SWE-Verified 95%（leaderboard 首位）、SWE-Pro 80%、OSWorld-Verified 85%、GPQA Diamond 88.5%、HLE 53%（首位）。$10/$50。復活後は週次50%上限があり、採用判断はレビュー記事を参照", swe: 95, swePro: 80, terminalBench: null, osworld: 85, browseComp: null, gpqa: 88.5, aime: null, hle: 53 },
  { name: "Claude Fable 5.1", rating: 5, summary: "Anthropic 一般公開フラッグシップの更新版（9/1）。破壊的変更が3つあり既存コードが 400 で止まりうる。キャッシュ読み取りが基本入力の0.025倍（他 Claude の1/4）。Terminal-Bench-Science 0.1 で52.6%（Anthropic 自社計測、標準誤差±3.5〜4.5pt）、Terminal-Bench 4.0 で55.8。TB 4.0 は表の他モデル（2.0/2.1）と版が違うため列には入れていない", swe: null, swePro: null, terminalBench: null, osworld: null, browseComp: null, gpqa: null, aime: null, hle: null },
  { name: "Claude Opus 5", rating: 5.0, summary: "Anthropic 現行フラッグシップ（7/24 GA）。Claude Max の既定モデル。effort トグル（low/medium/high）でタスクごとに思考量を選べる。ARC-AGI-3 で 30.2%（次点の約3倍）、1M コンテキスト。$5/$25 で Fable 5 の半額。SWE-bench Verified の公表値は情報源により 72.5〜97% と大きく開くため、当サイトは確定値として扱わない", swe: null, swePro: null, terminalBench: null, osworld: null, browseComp: null, gpqa: null, aime: null, hle: null },
  { name: "Claude Opus 5.5", rating: 5, summary: "Anthropic 現行の既定モデル（2026年9月22日公開）。新しい Claude 5.5 ファミリーの最初のモデルで、Sonnet 5.5 と Haiku 5.5 が数週間のうちに続くとされる。公式ドキュメントが「ほとんどのワークロードではここから始めよ」とし、Opus 5 は legacy へ移った。$4/$20 で Opus 5 から単価20%減、キャッシュ読み取りは $0.20（Opus 5 比60%減）。公式は「典型的なワークロードで40%安い」としており、内訳は単価減×1タスクあたりのトークン数減。生成は30%以上速い。1M コンテキスト、最大出力128K、知識カットオフ2026年6月。思考は常時オンで無効化不可、既定 effort は medium（Opus 5 は high）だが、同じ effort では Opus 5 より多く思考する。破壊的変更が4つある。公式の比較は全9項目で、Fable 5.1 には9項目すべて勝ち、GPT-6 Astra には2項目（AutomationBench 40.0 対 41.4、Terminal-Bench-Science 0.1 58.7 対 64.6）で負けている。Terminal-Bench 4.0 で66.4%、FrontierCode v1.1 Main 54.4%、CursorBench 4.0 57.8%、GDPval-AA v2.1 1846 Elo、OSWorld 2.0 81.8%。ただし表の公開スコアは原則 max effort（TB 4.0 のみ xhigh）である。一方で公式は既定 effort（medium）での値も別に示しており、FrontierCode 54.6%（max の 54.4% とほぼ同じ）、CursorBench 52.5%、TB 4.0 と GDPval-AA v2.1 は既定で Opus 5 や GPT-6 Astra の max に並ぶか上回るとしている。TB 4.0 は表の他モデル（2.0/2.1）と版が違うため列には入れていない", swe: null, swePro: null, terminalBench: null, osworld: null, browseComp: null, gpqa: null, aime: null, hle: null },
  ...
  { name: "GPT-6 Astra", rating: 5, summary: "OpenAI 最新フラッグシップ（9/3 限定プレビュー）。Preparedness Framework のサイバーセキュリティ区分で自社初の「Critical」判定。ExploitBench 100%（前世代 Sol は78.5%）だが直近3か月の新規脆弱性では39%。FrontierMath Tier 4 v2 97.6%、GPQA Diamond 96%、DeepSWE v1.1 74.1%。ARC-AGI-3 は測定条件で振れ、OpenAI 主張99.9% に対し ARC Prize の検証は専用アダプタ98.55% / provider 中立62.71%。$10/$50、272K トークン超は $20/$75。公式ページに到達できておらず、数値は独立した複数の二次ソースの一致で確認したものであるため列には入れていない", swe: null, swePro: null, terminalBench: null, osworld: null, browseComp: null, gpqa: null, aime: null, hle: null },
  { name: "GPT-6 Sol", rating: 4.5, summary: "OpenAI 中位モデル（9/22公開）。GPT-6 Astra譲りの設計で105万トークンctx、最大出力12.8万。$2/$10でGPT-5.6 Sol比半額以下。OSWorld 2.0（xhigh）60.5%はClaude Opus 5（medium）60.3%とほぼ並ぶが、公式ページに到達できておらず独立した複数の二次ソースの一致で確認した数値のため列には入れていない", swe: null, swePro: null, terminalBench: null, osworld: null, browseComp: null, gpqa: null, aime: null, hle: null },
  { name: "GPT-6 Luna", rating: 4, summary: "OpenAI 軽量モデル（9/22公開）。GPT-6 Astra譲りの設計で105万トークンctx。$0.10/$0.50でGPT-5.6 Luna比半額以下。知識カットオフは5/18でSolやAstraより新しい。OSWorld 2.0（max）でGPT-5.6 Sol（medium）を上回るとされるが、公式ページに到達できておらず独立した複数の二次ソースの一致で確認した内容のため列には入れていない", swe: null, swePro: null, terminalBench: null, osworld: null, browseComp: null, gpqa: null, aime: null, hle: null },
  ...
  { name: "Grok 4.7", rating: 4.5, summary: "xAI 最新フラッグシップ（9/21公開）。単価・コンテキスト（500K）は4.6から据え置き（$2/$6、200K超で$4/$12）。Musk氏によればパラメータ2.1T（4.6の1.5Tから+40%、xAI公式発表に記載はなく独立検証なし）。Artificial Analysisが同時期にTerminal-Bench 4.0を新規採用して指標を刷新したため、旧版でのGrok 4.6のスコア（61）とは比較不可。刷新後の指標でIntelligence Index 46（Fable 5.1・GPT-6 Astraはともに53）、Terminal-Bench 4.0 26%（GPT-6 Astra 約60%、Fable 5.1 約55%）、CursorBench 4.0 46.3%（GPT-5.6 Sol 41.7%は上回るがFable 5.1 51.8%には劣る）、DeepSWE v1.1 71.0%、GDPval Elo 1695（Fable 5.1 1735に次ぎGPT-6 Astra 1542の上）", swe: null, swePro: null, terminalBench: null, osworld: null, browseComp: null, gpqa: null, aime: null, hle: null },
  ...
  { name: "DeepSeek V4.1 Flash", rating: 4, summary: "DeepSeek 新アーキテクチャ第一弾（9/10）。552B total / 8B(入力)・16B(出力) active MoE、1M ctx、384K 出力、ネイティブ画像理解、MIT。9/14正午(北京時間)以降 V4 Pro宛リクエストを自動転送・Flash料金に統合。$0.15/$0.60(オフピーク、cache-hit $0.003)。エージェント系ベンチマークでGPT-5.6 Sol・Opus-5.0超えを主張するが自社発表値のみで独立検証なし", swe: null, swePro: null, terminalBench: null, osworld: null, browseComp: null, gpqa: null, aime: null, hle: null },
  ...
  { name: "GLM-5.3", rating: 4.5, summary: "Z.ai 新フラッグシップ（8/14）。GLM-5.2と同一のベース重みにポストトレーニングのみで強化、744B MoE（アクティブ40B）、1M ctx、Apache 2.0。Terminal-Bench 3.0 28.3・CyberGym最高水準・Agents' Last Exam(CLI) 28.5は自社発表値のみで独立検証なし、版が異なり旧Terminal-Bench 2.1の数値とは比較不可", swe: null, swePro: null, terminalBench: null, osworld: null, browseComp: null, gpqa: null, aime: null, hle: null },
  { name: "GLM-5.3-Flash", rating: 4, summary: "Z.ai 軽量版（8/26）。320B MoE（アクティブ18B）、疎+線形のハイブリッド注意機構、1M ctx、30T トークン追加事前学習、Apache 2.0でHugging Face/ModelScopeにFP8・BF16重み公開済み。ベンチマークは自社発表値のみ", swe: null, swePro: null, terminalBench: null, osworld: null, browseComp: null, gpqa: null, aime: null, hle: null },
