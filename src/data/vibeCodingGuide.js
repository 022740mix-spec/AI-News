...
export const VIBE_CLAUDE_CODE = {
  ...
  terms: [
    ...
    {
      word: "Auto モード（`Shift+Tab` で切り替え）",
      section: "権限モード",
      mean: "リスク分類で安全な操作の確認を減らすモード。auto は「全部スキップ」ではなく「低リスクだけスキップ」。**かつて必要だった `--enable-auto-mode` は v2.1.111 で削除され、いまは `Shift+Tab` のモード循環に既定で含まれる。** ==機能が無くなったのではなく、有効化の方法が変わった。== 起動時から bypassPermissions を循環に入れたい場合は `--allow-dangerously-skip-permissions` を使う。",
    },
    {
      word: "診断: `claude doctor`",
      section: "困ったとき",
      mean: "インストールと設定の診断結果を出す。動かない・設定が効かないときに最初に打つコマンド。==どこが壊れているかを自分で切り分ける前に、これを見る。==",
      code: "claude doctor",
    },
    {
      word: "設定を疑う: `--safe-mode`",
      section: "困ったとき",
      mean: "すべてのカスタマイズを無効にして起動する。CLAUDE.md・スキル・Hooks・MCP のどれかが原因かを切り分けられる。**素の状態で直るなら、自分の設定が原因。**",
      code: "claude --safe-mode",
    },
    {
      word: "設定を読まない: `--bare`",
      section: "困ったとき",
      mean: "Hooks・スキル・カスタムコマンドの自動発見をスキップする最小構成。`--safe-mode` より軽く、起動が遅いときの切り分けにも使える。",
      code: "claude --bare",
    },
    {
      word: "使いすぎ防止: `--max-budget-usd` / `--max-turns`",
      section: "コストと停止条件",
      mean: "API 呼び出しを止める上限額と、エージェントのターン数上限。==自動実行やスクリプトから回すときは、止まる条件を先に決めておく。== 想定外に回り続けたときの被害を金額で頭打ちにできる。",
      code: "claude -p \"...\" --max-budget-usd 5 --max-turns 20",
    },
    {
      word: "思考量: `--effort`",
      section: "コストと停止条件",
      mean: "そのセッションで使う思考の深さを指定する。深くすると質は上がるが、時間とトークンが増える。**難しい作業だけ上げ、定型作業では下げる**のが実際の使い分け。",
      code: "claude --effort high",
    },
    {
      word: "落ちたとき: `--fallback-model`",
      section: "コストと停止条件",
      mean: "主モデルが過負荷・利用不可のときに自動で切り替える先を指定する。長時間の自動実行では、止まるより切り替わったほうが良い場面がある。",
      code: "claude --fallback-model <model>",
    },
    {
      word: "並列実行: `claude agents` / `--bg`",
      section: "並列と自動化",
      mean: "`--bg` でバックグラウンドのエージェントとして開始し、`claude agents` で複数セッションを1画面で監視・操作する。`claude attach <id>` で手元のターミナルに引き寄せ、`claude logs <id>` で出力だけ見る。==1つのモデルに全部やらせず、分けて回すための道具。==",
      code: "claude --bg \"...\"   /   claude agents",
    },
    {
      word: "作業場所を分ける: `--worktree` / `--tmux`",
      section: "並列と自動化",
      mean: "git の worktree を切って作業を分離する。複数の変更を同時に走らせても、互いのファイルを踏まない。`--tmux` は worktree ごとに tmux セッションを作る（`--worktree` が前提）。",
      code: "claude --worktree",
    },
    {
      word: "定型出力: `--output-format` / `--json-schema`",
      section: "並列と自動化",
      mean: "スクリプトや CI から呼ぶときに、出力の形を固定する。`--json-schema` を渡すと、スキーマに沿った検証済み JSON が返る。**後段のプログラムで扱う前提なら、文章ではなく構造で受け取る。**",
      code: "claude -p \"...\" --output-format json",
    },
    {
      word: "CI 用の認証: `claude setup-token`",
      section: "並列と自動化",
      mean: "CI やスクリプトから使うための長期 OAuth トークンを発行する。==対話ログインができない環境で使う。== 発行したトークンは資格情報なので、リポジトリに置かず Secrets で渡す。",
      code: "claude setup-token",
    },
    {
      word: "他ツールからの移行: `claude import`",
      section: "困ったとき",
      mean: "Codex や Gemini CLI の設定を読み込んで取り込む。**乗り換えのたびに設定を書き直さなくてよい。**",
      code: "claude import codex",
    },
    {
...
export const GLOSSARY_BY_GENRE = [
  {
    ...
    terms: [
      ...
      {
        word: "コンテキストウィンドウ",
        mean: "モデルが一度に読み込める文章量の上限。トークンという単位で数え、「200K」なら約20万トークンまで入る。上限を超えた分は古いほうから落ちるか要約されるため、長い会話や大きなファイルを扱うと前半の内容が効かなくなる。上限は世代ごとに変わり、提供が打ち切られることもある（Anthropic は Sonnet 4.5/4 の100万トークン版を終了した）。大きいほど良いとは限らず、入れた量に応じて費用と応答時間も増える。",
      },
      {
        word: "トークン",
      ...
      {
        word: "蒸留（知識蒸留 / Distillation）",
        mean: "高い性能のモデル（教師）に大量の問いを投げ、返ってきた答えを集めて別のモデル（生徒）の学習に使う手法。教師の振る舞いを、より小さく安く動くモデルに移せる。軽量モデルを作る正攻法の一つで、手法そのものは機械学習で広く使われている。問題になるのは集め方のほうで、提供元の利用規約が出力の学習利用を禁じている場合や、不正なアカウントで地域・回数の制限を回避した場合に争いになる。公開された重みから作るのと、他社の API を叩いて集めるのとでは、規約上も法的にも扱いが違う。",
      },
      {
      ...
      {
        word: "マルチエージェント",
        mean: "1つの仕事を複数のエージェントに分担させる構成。調査・実装・レビューのように役割を分け、それぞれが自分のコンテキストを持つため、1つに全部やらせるより各自の文脈が短く保たれる。Claude Code や Gemini CLI ではサブエージェントとして提供されており、親から子に指示を渡して並列に動かす。受け渡す情報を絞りすぎると子が前提を知らないまま作業し、渡しすぎると分ける意味が無くなる。",
      },
      {
        word: "Computer Use（コンピュータ操作）",
        mean: "AI が画面を見て、マウスとキーボードを操作する仕組み。API が用意されていないアプリケーションでも、人が使う手順をそのまま実行させられる。エージェントの一形態にあたる。画面の見え方に依存するため、レイアウトの変更や読み込みの待ち時間で失敗しやすく、その安定度は OSWorld というベンチマークで測られる。モデル比較表の OSWorld 列がこの数値である。",
      },
      {
        word: "オープンウェイト",
        mean: "学習済みの重み（パラメータ）が公開されているモデル。ダウンロードして自分の環境で動かせるほか、ファインチューニングもできる。DeepSeek、Qwen、Kimi、GLM などが該当し、モデル比較表でも同じ指標で並べている。ただし学習に使ったデータや手順まで公開されているとは限らず、その点でオープンソースとは異なる。使ってよい範囲はモデルごとのライセンスで決まるため、商用利用の可否は導入前に必ず確認する。",
      },
      {
        word: "ローカル LLM",
        mean: "自分の PC やサーバーで動かす LLM。Ollama のような実行環境に、オープンウェイトのモデルを読み込んで使う。`ollama run` の一行で起動でき、入力が外部に出ずトークン課金も発生しない。ただし応答の速さと精度は手元の GPU に左右され、同じモデル名でも量子化（サイズを削る処理）の程度によって結果が変わる。",
      },
      {
        word: "エッジ AI",
        mean: "クラウドに送らず、手元の端末やその近くの機器でモデルを動かす方式。通信の往復が無いため応答が速く、データが外に出ない。対象はスマートフォン・カメラ・車載機器などで、そこに載る大きさまで小さくしたモデルが使われる。PC で言語モデルを動かす場合はローカル LLM と呼び分けることが多い。選ぶ際は精度そのものより、その機器のメモリに収まるかが先に効く。",
      },
      {
        word: "AI 検索",
        mean: "質問に対して Web やアップロードした資料を検索し、その結果をもとに文章で答える仕組み。従来の検索がリンクの一覧を返すのに対し、答えそのものを返す。仕組みは RAG と同じで、検索して集めた文章をモデルに読ませてから答えさせている。答えが正しいかは、根拠として示された出典を読者が自分で確かめられるかで決まるため、出典の付け方が製品ごとの差になる。社内文書を対象にしたものは企業向けの製品名で呼ばれることが多く、Web を対象にしたものと区別されないまま語られることがある。",
      },
      {
        word: "メモリ",
        mean: "同じ語が2つの意味で使われる。AI サービスの「メモリ」は、会話をまたいで利用者の情報を覚えておく機能を指す。もう一つは計算機の主記憶（RAM）で、モデルを動かすのに必要な容量の話をしている。前者は名前や好みを次の会話に持ち越せる代わりに、覚えた内容が意図しない話題に混ざることがあるため、何を覚えているかを設定画面で確認・削除できるかが実務上の分かれ目になる。後者は「メモリを6分の1に削減」のような形で出てきて、同じモデルをより小さな機器で動かせるという意味になる。",
      },
      {
        word: "推論",
        mean: "これも2つの意味で使われる。一つはモデルを動かして答えを出す処理そのもの（inference）、もう一つは答えを出す前に筋道を立てて考える能力（reasoning）で、「推論モデル」はこちらを指す。「1秒あたり何トークン」「推論用チップ」は前者の話で、速度と費用に効く。「推論性能が前世代の倍」は後者の話で、難しい問題を解けるかに効く。同じ記事に両方が出ることがあるため、どちらの話かは前後で見分ける。",
      },
      {
        word: "スキル（Skills）",
        mean: "エージェントに特定の作業手順を覚えさせる仕組み。手順を書いたファイル（`SKILL.md` など）を置いておくと、エージェントが必要な場面でそれを読んで従う。画面を録画するだけで手順を生成する機能も出てきており、Claude・Codex・Microsoft がそれぞれ実装している。注意すべきは、スキルが説明書ではなく実行される指示である点で、他人が公開したものを中身を読まずに入れると、そこに書かれた操作をエージェントがそのまま行う。導入の前に必ず中身を読む。",
      },
    ],
  },
  {
  ...
  },
  {
    ...
    terms: [
      ...
      {
        word: "IDE（統合開発環境）",
        mean: "Integrated Development Environment の略。コードを書く・実行する・間違いを探す、を1つの画面でまとめて行えるソフト。VS Code や JetBrains 系がこれにあたる。CLI がコマンドを打って操作するのに対し、IDE は画面上で操作する。AI ツールは同じ機能が IDE の拡張機能としても CLI としても提供されていることがあり、エディタとエージェントは1対1ではない。使っているエディタを変えずにエージェントだけ入れ替えられる場合が多い。",
      },
      {
        word: "シェル",
  ...
  },
  {
    ...
    terms: [
      ...
      {
        word: "SDK（Software Development Kit）",
        mean: "ある API を自分のプログラムから使いやすくするために、提供元が用意した部品一式。API を直接呼ぶ場合は HTTP リクエストを自分で組み立てるが、SDK を使うと言語ごとの関数呼び出しで済む。認証やリトライ、データの型の定義まで含まれることが多い。自分が使う言語の公式 SDK が用意されているかどうかは、そのサービスを採用する際の判断材料になる。",
      },
      {
        word: "サーバーレス",
  ...
  },
  {
    ...
    terms: [
      ...
      {
        word: "埋め込み（エンベディング）",
        mean: "テキストや画像を、意味の近さが数値の近さになるように変換したベクトル。専用の埋め込みモデルが変換を行い、「犬」と「子犬」は数値としても近く、「犬」と「会計」は遠くなる。ベクトル検索も RAG も、この変換を前提にして成り立っている。変換に使ったモデルを途中で差し替えると、すでに保存したベクトルとは比較できなくなるため入れ直しが要る。次元数や対応言語がモデルごとに違う点も、選ぶときに効く。",
      },
      {
        word: "ベクトル検索",
      ...
      {
        word: "ベクトルデータベース（ベクトル DB）",
        mean: "ベクトルに変換したデータを保存し、類似度で高速に検索できるようにしたデータベース。ベクトル検索という手法を、製品として使える形にしたもの。RAG を組むときは、社内文書を分割して入れておく置き場になる。Pinecone や Weaviate のような専用製品のほか、PostgreSQL に pgvector を入れて既存のデータベースのまま済ませる方法もある。件数が少ないうちは後者で足りるため、規模と運用の手間で選び分ける。",
      },
      {
  ...
  },
  {
    ...
    ref: {
      ...
      terms: [
        ...
        // ── スラッシュコマンド（CLI） ──
        { word: "操作: `/model`", section: "スラッシュコマンド", mean: "使用するモデルを切り替える。GPT-5.4 系の各バリアント（Standard, Thinking, Pro）やコーディング特化モデルを選べる。Claude Code の `/model` と同じ役割。" },
        { word: "操作: `/compact`", section: "スラッシュコマンド", mean: "長い会話を要約して圧縮する。Claude Code の `/compact` と同じ。確認ダイアログが出るので Y で実行。コンテキストが足りなくなったときに使う。" },
        { word: "操作: `/diff`", section: "スラッシュコマンド", mean: "Git の差分を CLI 内で確認する。ステージ済み・未ステージ・未追跡ファイルをまとめて表示。コミット前の確認に使う。Claude Code の `/diff` と同じ。" },
        { word: "操作: `/review`", section: "スラッシュコマンド", mean: "ワーキングツリーの変更を AI がレビューし、問題点やテスト不足を指摘する。PR を出す前のセルフレビューに便利。" },
        { word: "操作: `/plan`", section: "スラッシュコマンド", mean: "Plan Mode に切り替える。変更前に AI が計画を提示し、承認してから実行する。Claude Code の `/plan` と同じ。" },
        { word: "操作: `/permissions`", section: "スラッシュコマンド", mean: "承認プリセットを変更する。Auto（全自動）/ Read Only（読み取りのみ）等を選択。Claude Code の `/permissions` に相当。" },
        { word: "操作: `/fork`", section: "スラッシュコマンド", mean: "現在の会話を新しいスレッドにクローンする。元の会話は残したまま、別のアプローチを並行して試せる。Claude Code にはない Codex 独自の機能。" },
        { word: "操作: `/mention`", section: "スラッシュコマンド", mean: "`/mention ファイルパス` で特定のファイルをコンテキストに追加する。Claude Code の `@ファイル名`、Cursor の `@file` と同じ役割。" },
        { word: "操作: `/new`", section: "スラッシュコマンド", mean: "同じ CLI セッション内で新しい会話を開始する。Claude Code の `/clear` に近い。" },
        { word: "操作: `/clear`", section: "スラッシュコマンド", mean: "ターミナルをクリアし、会話をリセットする。`/new` と似ているが画面もリセットされる。" },
        { word: "操作: `/fast`", section: "スラッシュコマンド", mean: "高速モードに切り替える。Claude Code の `/fast` と同じ概念。" },
        { word: "カスタムスラッシュコマンド", section: "スラッシュコマンド", mean: "チームや個人で再利用可能なプロンプトを `/コマンド名` として登録できる。Cursor の `.cursor/commands/` や Claude Code の Skills と同じ仕組み。" },
        // ── クラウド実行 ──
  ...
  },
  {
    ...
    ref: {
      ...
      terms: [
        ...
        // ── スラッシュコマンド（Chat 内） ──
        { word: "==Copilot のスラッシュコマンド==", section: "スラッシュコマンド", mean: "Copilot Chat 内で `/` を打つと利用可能なコマンド一覧が表示される。コードの説明・修正・テスト生成などの定型タスクを一発で指示できる。長いプロンプトを書かなくて済む。" },
        { word: "`/explain` — コードを説明", section: "スラッシュコマンド", mean: "選択したコードの動作を AI が解説する。「この関数は何をしているのか」を素早く把握したいときに使う。最もよく使うコマンド。", code: "/explain", codeLang: "text" },
        { word: "`/fix` — エラーを修正", section: "スラッシュコマンド", mean: "選択したコードのバグやエラーを AI が自動修正する。エラーの波線が出ているコードを選択して `/fix` が基本パターン。", code: "/fix", codeLang: "text" },
        { word: "`/tests` — テスト生成", section: "スラッシュコマンド", mean: "選択した関数やクラスのユニットテストを自動生成する。エッジケースやモックも含めて生成してくれる。テストフレームワークは `copilot-instructions.md` で指定すると適切なものが使われる。", code: "/tests", codeLang: "text" },
        { word: "`/doc` — ドキュメント生成", section: "スラッシュコマンド", mean: "選択した関数やクラスの JSDoc / docstring を自動生成する。既存コードにドキュメントを後付けしたいときに便利。", code: "/doc", codeLang: "text" },
        { word: "`/new` — 新しいファイル作成", section: "スラッシュコマンド", mean: "自然言語の指示から新しいファイルやプロジェクトのひな形を生成する。「Express の REST API サーバーを作って」のように使う。", code: "/new Express の REST API サーバー", codeLang: "text" },
        { word: "==`/` `@` `#` の違い==", section: "スラッシュコマンド", mean: "`/` はタスク指示（/fix, /explain 等）。`@` はスコープ指定（@workspace, @terminal 等で AI の参照範囲を決める）。`#` はファイル参照（#file:パスで特定ファイルを渡す）。3つのプレフィックスで役割が分かれている。Gemini CLI の `/ @ !` や Cursor の `/ @` と同じ設計思想。" },
  ...
  },
  {
    ...
    ref: {
      ...
      terms: [
        ...
        // ── 設定 ──
        // ── スラッシュコマンド横断比較 ──
        { word: "==スラッシュコマンドの仕組みの違い==", section: "スラッシュコマンド", mean: "ツールによって `/` の意味が全く違う。\nClaude Code: ==組み込みのセッション操作==（/compact, /model, /diff 等）。ツール本体に機能が内蔵されている。\nCursor: ==ユーザー定義のプロンプトテンプレート==（.cursor/commands/ に Markdown を置いて自作）。Claude Code の Skills に近い。\nCodex CLI: ==組み込みコマンド＋カスタムコマンド==の両方に対応。Claude Code に最も近い。\nGemini CLI: ==組み込みコマンド==（/compact, /model, /stats 等）。Claude Code と同じ設計。\nCopilot: ==タスク指示ショートカット==（/fix, /explain, /tests 等）。コードに対するアクションを一発で指示する。" },
        { word: "会話を圧縮する", section: "スラッシュコマンド", mean: "Claude Code: `/compact`\nCodex CLI: `/compact`\nGemini CLI: `/compact`\nCursor: 新しい Chat を開始する（圧縮コマンドはない）\nCopilot: 新しい Chat を開始する（圧縮コマンドはない）" },
        { word: "モデルを切り替える", section: "スラッシュコマンド", mean: "Claude Code: `/model`\nCodex CLI: `/model`\nGemini CLI: `/model`\nCursor: `Cmd+Shift+P` → モデル選択（コマンドパレット経由）\nCopilot: `Cmd+Shift+P` → モデル選択（コマンドパレット経由）" },
        { word: "変更の差分を確認する", section: "スラッシュコマンド", mean: "Claude Code: `/diff`\nCodex CLI: `/diff`\nGemini CLI: —（`!git diff` でシェル経由）\nCursor: Source Control パネル（エディタ UI）\nCopilot: Source Control パネル（エディタ UI）" },
        { word: "計画モードに切り替える", section: "スラッシュコマンド", mean: "Claude Code: `/plan`（Shift+Tab でも可）\nCodex CLI: `/plan`\nGemini CLI: Plan Mode（設定で切替）\nCursor: Composer の Plan Mode（モードドロップダウン）\nCopilot: —（計画モードはない）" },
        { word: "AI にコードをレビューさせる", section: "スラッシュコマンド", mean: "Claude Code: —（自然言語で指示）\nCodex CLI: `/review`（ワーキングツリー自動レビュー）\nGemini CLI: —（自然言語で指示）\nCursor: Bug Finder（`Cmd+Shift+P` → bug finder）\nCopilot: `/fix` または PR での自動レビュー（Business 以上）" },
        { word: "ファイルをコンテキストに追加する", section: "スラッシュコマンド", mean: "Claude Code: `@ファイル名`\nCodex CLI: `/mention ファイルパス`\nGemini CLI: `@ファイル名`\nCursor: `@file`\nCopilot: `#file:ファイル名`\n==プレフィックスがツールごとに異なる点に注意==。" },
        { word: "==プレフィックス記号の対応表==", section: "スラッシュコマンド", mean: "各ツールは `/` `@` `#` `!` を異なる意味で使う。\n`/`（スラッシュ）: Claude Code=操作 / Cursor=プロンプト / Codex=操作 / Gemini=操作 / Copilot=タスク指示\n`@`（アット）: Claude Code=ファイル / Cursor=コンテキスト / Codex=なし / Gemini=ファイル / Copilot=スコープ\n`#`（ハッシュ）: Copilot のみ=ファイル参照\n`!`（エクスクラメーション）: Gemini CLI のみ=シェル直接実行\n==同じ記号でもツールによって意味が違うので、ツールを切り替えるときは注意==。" },
        // ── 設定 ──
