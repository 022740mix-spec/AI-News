...
export const TOOL_REFERENCES = [
  ...
  },
  {
    ...
    ref: {
      ...
      terms: [
        // ── 基本操作 ──
        { word: "インライン編集: `Cmd+K`（Mac）/ `Ctrl+K`（Win）", mean: "エディタ上でコードを選択して Cmd+K を押すと、AI への指示入力欄が出る。「この関数をリファクタして」「エラーハンドリングを追加して」のような短い修正に向く。" },
        { word: "Composer: `Cmd+I`（Mac）/ `Ctrl+I`（Win）", mean: "Cmd+I で Composer パネルが開く。複数ファイルをまたぐ大きな変更を対話的に進める。パネル内のドロップダウンで「Agent」を選択すると、AI が自律的にファイル探索・編集・ターミナル実行まで行う。" },
