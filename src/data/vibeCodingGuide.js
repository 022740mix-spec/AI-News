...
export const GLOSSARY_BY_GENRE = [
  {
    ...
    terms: [
      ...
      {
        word: "マルチエージェント",
        mean: "1つの仕事を複数のエージェントに分担させる構成。調査・実装・レビューのように役割を分け、それぞれが自分のコンテキストを持つ。1つのエージェントに全部やらせるより各自の文脈が短く保たれるが、受け渡しの設計を誤ると同じ作業の重複や指示の取り違えが起きる。",
      },
      {
        word: "Computer Use（コンピュータ操作）",
        mean: "AI が画面を見て、マウスとキーボードを操作する仕組み。API が用意されていないアプリケーションでも、人が使う手順をそのまま実行させられる。エージェントの一形態だが、画面の見え方に依存するため、レイアウトの変更や読み込みの待ち時間で失敗しやすい。",
      },
      {
        word: "オープンウェイト",
        mean: "学習済みの重み（パラメータ）が公開されているモデル。ダウンロードして自分の環境で動かせるほか、ファインチューニングもできる。ただし学習に使ったデータや手順まで公開されているとは限らず、その点でオープンソースとは異なる。利用できる範囲はモデルごとのライセンスで決まり、商用利用や再配布が制限されることもある。",
      },
      {
        word: "ローカル LLM",
        mean: "自分の PC やサーバーで動かす LLM。Ollama のような実行環境に、オープンウェイトのモデルを読み込んで使う。入力が外部に出ずトークン課金も発生しないが、応答の速さと精度は手元の GPU の性能に左右される。",
      },
      {
        word: "エッジ AI",
        mean: "クラウドに送らず、手元の端末やその近くの機器でモデルを動かす方式。通信の往復が無いため応答が速く、データが外に出ない。スマートフォンやカメラなど計算資源の限られた機器が対象になるため、そこで動く大きさに収めた小型のモデルが使われる。ローカル LLM は、このうち言語モデルを PC で動かす場合を指す。",
      },
    ],
  },
  {
